...
# ============ 夹爪和安全参数 ============
GRIPPER_LENGTH = 0.17           # 夹爪长度 17cm
MIN_END_HEIGHT = 0.28           # 末端最小安全高度 28cm (考虑17cm夹爪+安全余量)
...
def estimate_end_effector_height(j2, j3, j4):
    ...
    L3 = 0.1735   
    L4 = 0.08     
    L5 = GRIPPER_LENGTH  # 夹爪长度 17cm
    angle2 = j2_rad
    angle3 = angle2 + j3_rad
    angle4 = angle3 + j4_rad

    height = 0.155 + L1  # base高度 + base到joint2
    height += L2 * math.cos(angle2)  # 大臂贡献
    height += L3 * math.cos(angle3)  # 小臂贡献
    height += L4 * math.cos(angle4)  # 腕部贡献
    height += L5 * math.cos(angle4)  # 夹爪贡献（与腕部同方向）
...
def estimate_end_effector_distance_to_base(j2, j3, j4):
    ...
    # 计算水平距离（XY平面）
    distance = abs(L2 * math.sin(angle2))
    distance += abs(L3 * math.sin(angle3))
    distance += abs(L4 * math.sin(angle4))
    distance += abs(L5 * math.sin(angle4))
...
def monitor_height():
    ...
    MIN_SAFE_HEIGHT = 170  # 最小安全高度 170mm
    RECOVERY_HEIGHT = 170  # 恢复高度阈值 170mm (高于此高度自动恢复)
    last_warning_time = 0
